...
- (id)initWithFrame:(NSRect)frameRect
{
	...
	{
		//NSLog(@"frameRect x: %2.1f y: %2.1f h: %2.1f w: %2.1f",frameRect.origin.x,frameRect.origin.y,frameRect.size.height,frameRect.size.width);
...
- (void)setLevel:(int) derLevel
{
	...
	if ((Level<=lastLevel)&&(Level>0)&&(maxSet==NO))
	{
		...
		//NSLog(@"Bedingung A: lastLevel: %2.2f   Level: %2.2f  max: %2.2f", lastLevel,Level,max);
		max=lastLevel;
